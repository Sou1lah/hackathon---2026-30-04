...
import { 
  Activity, FileText, CheckSquare, AlertTriangle, 
  Loader2, Clock, ShieldAlert, BarChart3,
  Server, CreditCard, Archive, ExternalLink, ArrowUpRight
} from "lucide-react"
    ...
    <div className="space-y-8 p-2 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-4xl font-black italic tracking-tighter text-primary">Dashboard</h1>
            <p className="text-muted-foreground italic font-medium">Moteur de décision & Métriques système.</p>
          </div>
        </div>
        <div className="text-right">
          <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest italic">Dernière mise à jour</p>
          ...
          <div className="bg-card border border-border rounded-[32px] shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-border">
              {data.alerts.length === 0 ? (
                <div className="p-12 text-center text-muted-foreground italic">Aucune alerte critique détectée. Système nominal.</div>
              ) : (
                data.alerts.map((alert) => (
                  <div key={alert.id} className="p-6 flex items-start gap-4 hover:bg-accent/30 transition-colors group">
                    <div className={cn(
                      "p-2 rounded-xl mt-1",
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-black text-xs uppercase tracking-widest">{alert.type}</span>
                        <span className="text-[10px] font-bold text-muted-foreground italic">{new Date(alert.created_at).toLocaleString()}</span>
                      </div>
                      <p className="font-medium text-sm text-foreground/80">{alert.message}</p>
                      {alert.dossier_id && (
          ...
          <div className="bg-card border border-border rounded-[32px] shadow-sm p-8 flex flex-col items-center">
             ...
             <div className="flex justify-center gap-6 mt-4 w-full">
                <div className="text-center">
                  <p className="text-[10px] font-black text-muted-foreground uppercase italic tracking-widest mb-1">On Time</p>
                  <p className="text-xl font-black text-emerald-500">{data.sla.on_time_count}</p>
                </div>
                <div className="text-center">
                  <p className="text-[10px] font-black text-muted-foreground uppercase italic tracking-widest mb-1">Breached</p>
                  <p className="text-xl font-black text-destructive">{data.sla.breached_count}</p>
                </div>
             </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-[32px] shadow-sm overflow-hidden">
            ...
            <div className="p-4 space-y-3">
              {data.internships.latest_internships.map((offer) => (
                <div key={offer.id} className="p-3 bg-accent/20 rounded-2xl hover:bg-accent/40 transition-colors cursor-pointer group">
                  <p className="font-bold text-sm line-clamp-1 group-hover:text-primary transition-colors">{offer.title}</p>
            ...
            <div className="space-y-6">
              {data.system_health.map((sys) => (
                <div key={sys.name} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    {sys.name.includes("Payment") ? <CreditCard size={18} className="text-muted-foreground" /> : 
                     sys.name.includes("Archive") ? <Archive size={18} className="text-muted-foreground" /> :
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className={cn(
                      ...
                      sys.status === 'degraded' ? "bg-amber-500 shadow-amber-500/50" : "bg-red-500 shadow-red-500/50"
                    )} />
                  </div>
                </div>
              ))}
...
function StatCard({ icon, label, value, subtext, color, alert = false }: { 
  ...
  return (
    ...
    >
      <div className={cn("p-4 w-fit rounded-2xl shadow-inner", colorMap[color])}>
        {icon}
      </div>
      <div>
        <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest italic mb-1">{label}</p>
        <p className="text-3xl font-black tracking-tight">{value}</p>
        <p className="text-xs text-muted-foreground italic mt-1 font-medium">{subtext}</p>
      </div>
      {alert && (
