      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Gestion et suivi administratif des signatures électroniques PAdES.
          </p>
        </div>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col min-h-[800px]">
            <div className="bg-accent/50 border-b border-border p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-sm">
                    Convention_ElBouni_2024.pdf
                  </h3>
                  <p className="text-[10px] text-muted-foreground font-medium">
                    MODIFIÉ IL Y A 2 HEURES • PDF/A-3
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button className="bg-foreground text-background px-4 py-2 rounded-lg text-xs font-bold hover:opacity-80 transition-all">
                  Signer Maintenant
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 bg-accent/30 p-12 overflow-y-auto flex justify-center">
              <div className="w-full max-w-2xl bg-card aspect-[1/1.414] shadow-2xl rounded p-16 space-y-8 relative group border border-border">
                <div className="absolute inset-0 bg-foreground/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-zoom-in rounded">
                  <span className="bg-card px-4 py-2 rounded-full font-bold text-sm shadow-xl border border-border">
                ...
                <div className="text-center space-y-2 py-4">
                  <h1 className="text-xl font-serif font-bold uppercase tracking-widest">
                    CONVENTION DE STAGE
                  </h1>
                ...
                <div className="space-y-4 text-[13px] leading-relaxed">
                  ...
                  <p>
                    L'Institution dénommée [NOM INSTITUTION], représentée par M.
                    le Recteur, ci-après désignée « L'Université ».
                  </p>
                  <p className="font-bold">ET :</p>
                  <p>
                    L'Entreprise [NOM ENTREPRISE], sise à [ADRESSE], représentée
                    par [REPRÉSENTANT].
                  </p>
                </div>
                <div className="pt-8 border-t border-border">
                  <h4 className="font-bold text-xs mb-4">
                    ARTICLE 1 : OBJET DE LA CONVENTION
                  </h4>
                  <p className="text-[12px] text-muted-foreground italic">
                    La présente convention règle les rapports entre
                    l'Université, l'Entreprise et l'étudiant(e).
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-8 pt-20">
                  <div className="border-t border-muted-foreground/30 pt-2 opacity-40 italic text-[10px]">
          ...
          <div className="bg-card p-6 rounded-2xl border border-border shadow-sm sticky top-8">
                ...
                <div key={i} className="flex gap-4 group">
                  <div className="flex flex-col items-center">
                    ...
                    {i < SIGNATURE_STEPS.length - 1 && (
                      <div
                        className={cn(
                          "w-px h-full min-h-[30px] my-1 transition-colors",
                          step.status === "completed"
                            ? "bg-green-500"
                            : "bg-border",
                        )}
                  ...
                  <div className="pb-4">
                    ...
                    {step.status === "completed" && (
                      <p className="text-[9px] text-green-500 font-bold mt-1 italic uppercase">
                        SIGNÉ LE 29/04 14:20
                      </p>
                    )}
                    {step.status === "current" && (
                      <div className="mt-2 flex gap-2">
                        <button className="bg-primary text-primary-foreground text-[10px] font-bold px-3 py-1 rounded hover:opacity-80">
                          Rappeler
                        </button>
                        <button className="bg-accent text-muted-foreground text-[10px] font-bold px-3 py-1 rounded hover:bg-accent/80">
                          Détails
                        </button>
                      </div>
                    )}
            ...
            <div className="mt-8 p-4 bg-accent/50 rounded-xl border border-border">
              ...
              <p className="text-[10px] text-muted-foreground mt-3 italic text-center leading-relaxed">
                Ce document nécessite l'authentification forte via Certificat
                National ou Clé USB Token.
              </p>
            </div>
          </div>
        </div>
