...
import { ExternalLink, Calendar, Briefcase, RefreshCcw } from "lucide-react"
...
export default function InternshipOffers() {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        <p className="text-red-500 mb-4">Error loading offers: {(error as Error).message}</p>
  ...
  return (
    <div className="container mx-auto py-8 px-4 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Latest cooperation and internship opportunities from the official source.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {data?.data.map((offer) => (
          <Card key={offer.id} className="group relative flex flex-col overflow-hidden border-none shadow-sm hover:shadow-xl transition-all duration-300 bg-card/50 backdrop-blur-sm ring-1 ring-border/50 hover:ring-primary/20">
            ...
            <CardHeader>
              ...
              <CardTitle className="line-clamp-2 text-lg leading-snug group-hover:text-primary transition-colors cursor-pointer" dir="rtl">
                {offer.title}
              </CardTitle>
            </CardHeader>
            ...
            <CardFooter className="pt-0">
              <Button asChild variant="secondary" className="w-full group/btn overflow-hidden relative">
